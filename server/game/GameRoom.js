...
class GameRoom {
  ...
  _getUncommittedGuns(player) {
    if (!player) return 0;
    const committed = this.mutiny ? (this.mutiny.commitments.get(player.id) || 0) : 0;
    return Math.max(0, player.guns - committed);
  }
  ...
  _finishMutiny(newCaptainId) {
    ...
    for (const [pid, n] of this.mutiny.commitments) {
      ...
      const oldGuns = p.guns;
      p.guns = Math.max(0, p.guns - lost);
      console.log(`[GUNS] ${p.name}: ${oldGuns} -> ${p.guns} (Reason: mutiny lost ${lost})`);
      this._log(`[سلاح] ${p.name}: تم خصم ${lost} مسدسات (الرصيد: ${p.guns})`);
  ...
  _executeCardAction(card) {
    ...
    switch (card.action) {
      ...
      case C.NAV_ACTIONS.DISARMED: {
        const nav = this.player(this.navigatorId);
        if (nav && nav.guns > 0) {
          const oldGuns = nav.guns;
          nav.guns = Math.max(0, nav.guns - 1);
          this.generalSupply += 1; // [إصلاح #14]
          console.log(`[GUNS] ${nav.name}: ${oldGuns} -> ${nav.guns} (Reason: disarmed)`);
          this._log(`${nav.name} (Navigator) فقد مسدساً (الرصيد: ${nav.guns})`);
        }
        return tryFinish();
      }
  ...
  activateCharacter(playerId, payload = {}) {
    ...
    switch (ch.id) {
      case 'kleptomaniac': {
        ...
        const targetAvailable = this._getUncommittedGuns(t2);
        if (targetAvailable < 1) throw new Error('الهدف لا يملك مسدسات غير ملتزم بها في المطيانة');
        const oldTargetGuns = t2.guns;
        t2.guns = Math.max(0, t2.guns - 1);
        p.guns += 1;
        console.log(`[GUNS] Kleptomaniac: ${t2.name} (${oldTargetGuns} -> ${t2.guns}) stolen by ${p.name}`);
        this._log(`[سلاح] سرقة مسدس من ${t2.name} (رصيده: ${t2.guns}) إلى ${p.name} (رصيده: ${p.guns})`);
        ch.revealed = true;
      ...
      case 'gunsmith': {
        ...
        const oldGuns = p.guns;
        p.guns = Math.max(0, p.guns - 1); this.generalSupply += 1; // [إصلاح #14]
        console.log(`[GUNS] ${p.name}: ${oldGuns} -> ${p.guns} (Reason: gunsmith activation)`);
        this._log(`[سلاح] خصم مسدس لتفعيل صانع الأسلحة لـ ${p.name} (الرصيد: ${p.guns})`);
      ...
      case 'spiritualist': {
        ...
        if (!qa?.alive || !qb?.alive) throw new Error('لاعب غير صالح');
        if (this._getUncommittedGuns(qa) < 1 || this._getUncommittedGuns(qb) < 1)
          throw new Error('كلا اللاعبَين يجب أن يملكا مسدساً واحداً على الأقل غير ملتزم به في المطيانة');
        for (const id of [a, b]) {
          const q = this.player(id);
          if (q && q.guns > 0) {
            const oldGuns = q.guns;
            q.guns = Math.max(0, q.guns - 1); recv.guns += 1;
            console.log(`[GUNS] Spiritualist: ${q.name} (${oldGuns} -> ${q.guns}) to ${recv.name}`);
            this._log(`[سلاح] تأثير الروحاني: خصم مسدس من ${q.name} (الرصيد: ${q.guns}) إلى ${recv.name}`);
          }
        }
        ch.revealed = true;
      ...
      case 'debt_collector': {
        ...
        let availableToGive = this._getUncommittedGuns(t2);
        if (availableToGive < 1) throw new Error('المستهدف لا يملك مسدسات غير ملتزم بها ليدفع ديونه');
        ...
        for (const id of team) {
          if (id !== payload.targetId && availableToGive > 0 && t2.guns > 0) {
            availableToGive--;
            const oldGuns = t2.guns;
            t2.guns = Math.max(0, t2.guns - 1);
            const rx = this.player(id);
            rx.guns += 1;
            console.log(`[GUNS] Debt Collector: ${t2.name} (${oldGuns} -> ${t2.guns}) to ${rx.name}`);
            this._log(`[سلاح] جابي الديون: خصم مسدس من ${t2.name} (الرصيد: ${t2.guns}) إلى ${rx.name}`);
          }
        }
        ch.revealed = true;
        setLastActivated(`اللاعب المستهدف: ${t2.name}`);
      ...
      case 'instigator': {
        ...
        const gunsToCommit = this._getUncommittedGuns(target);
        if (gunsToCommit < 1) throw new Error('الهدف لا يملك مسدسات غير ملتزم بها ليضعها في المطيانة');
        ...
        target.guns = 0;
        console.log(`[GUNS] Instigator: ${target.name} set to 0 guns (Reason: forced by ${p.name})`);
        this._log(`[سلاح] المحرّك الخفي: إجبار ${target.name} على وضع مسدساته (${gunsToCommit}) وتصفير رصيده`);
  ...
  narratorOverride(action, payload = {}) {
    switch (action) {
      ...
      case 'set_guns': {
        const p = this.player(payload.playerId);
        if (p) {
          const oldGuns = p.guns;
          p.guns = Math.max(0, payload.guns);
          console.log(`[GUNS] Narrator set ${p.name} guns: ${oldGuns} -> ${p.guns}`);
          this._log(`[سلاح] أمر الراوي: ضبط مسدسات ${p.name} من ${oldGuns} إلى ${p.guns}`);
        }
        break;
      }
